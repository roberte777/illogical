...
/// and when it leaves.
///
/// An `NSTrackingArea` rather than SwiftUI's `onContinuousHover`, for one
/// measured reason: `onContinuousHover` stops delivering once a drag gesture
/// that began inside the view ends, and does not start again until the pointer
/// leaves the view and comes back. On the tab strip that is precisely the
/// moment hover matters, because every drop ends a drag under the pointer — so
/// the strip came out of a reorder frozen, with the ✕ stuck on the dropped tab
/// wherever the pointer went next.
///
/// A tracking area has no such gap: it is independent of hit testing and of
/// gestures, as this file's header already notes from the other direction, and
/// `mouseMoved` resumes on the first movement after the button comes up. The
/// options are the ones ghostty's surface uses (`SurfaceView_AppKit.swift`),
/// for the same reasons — `.inVisibleRect` so the clipped part of a scrolled
/// strip is not tracked, and `.activeAlways` so a window that is not key still
/// reports.
struct PointerTracker: NSViewRepresentable {
    ...
    final class TrackingView: NSView {
        ...
        /// Never the view a click lands on. This goes in as a `.background`
        /// under live controls, and a tracking area does not need to be the
        /// hit-test result to report — which is the whole reason it can be put
        /// somewhere this unobtrusive.
        override func hitTest(_ point: NSPoint) -> NSView? { nil }

        override func updateTrackingAreas() {
            super.updateTrackingAreas()
            // Recreated rather than resized, which is what AppKit calls this
            // for. `.inVisibleRect` ignores the rect and follows the view, so
            // the value passed here only matters before the first layout.
